...
namespace XRayAnalyzer.Controls
{
    ...
    public partial class PlotControl : UserControl
    {
        ...
        public IPlottable? PlotSpectrum(List<SignalPoint>? spectrumPoints)
        {
            ...
            if (XInChannels_IsSelected)
            {
                spectrumPlot.Xs = spectrumPoints.Select(point => (double)point.Channel).ToArray();
            }
            else if (XInEnergies_IsSelected) 
            { 
                spectrumPlot.Xs = spectrumPoints.Select(point => point.Energy).ToArray();
            }

            if (LinearScale_IsSelected)
            {
                
                spectrumPlot.Ys = spectrumPoints.Select(point => (double)point.Value).ToArray();
                string logTickLabels(double y) => y.ToString();
                WpfPlotControl.Plot.YAxis.TickLabelFormat(logTickLabels);
                WpfPlotControl.Plot.YAxis.MinorLogScale(false);
            }
            else if (LogarithmicScale_IsSelected)
            {
                spectrumPlot.Ys = spectrumPoints.Select(point => point.ValueLog).ToArray();
                string logTickLabels(double y) => Math.Pow(10, y).ToString();
                WpfPlotControl.Plot.YAxis.TickLabelFormat(logTickLabels);
                WpfPlotControl.Plot.YAxis.MinorLogScale(true);
            }
            ...
            WpfPlotControl.Plot.XLabel(xAxisOptions.SelectedValue.ToString());
            if (Settings.Default.Language == "LanguageSK") 
            {
                WpfPlotControl.Plot.YLabel("Početnosť");
            } else 
            {
                WpfPlotControl.Plot.YLabel("Counts");
            }
